...
##### Stance Controller Plots #####
    function PlotStanceGRF(t_all,u,p, title,sw)
        PlotTransitionLines(t_all, -p.kₛ*u_all[:,10] - p.cₛ*u[:,12], sw, "",L"t, \text{[s]}", L"GRF, \text{[N]}")
...
##### Stance Controller Plots #####
    function PlotAngularMomentum(t_all, u_all, sw, p, title, xlab, ylab)
        ...
        p1 = PlotTransitionLinesColor(t_all, H[1,:], sw, title, "", L"H_1, \text{[N-m-s]}")
        p2 = PlotTransitionLinesColor(t_all, H[2,:], sw, " ", xlab, L"H_2, \text{[N-m-s]}")
        plot(p1, p2, layout=(2,1),extra_plot_kwargs = KW(:include_mathjax => "cdn"))
    end
...
##### Import Data #####
(t_all,u_all,p,τ_ff,I,θd,sw,state) = MF5.ImportExpData(5)


# Generate some data
x,y = MF5.X_Y(u_all,p)
(τ, GRF) = MF5.MotorCurrentInterpret(p,I,u_all)
plotlyjs()

# Plotting Parameters
swplot = TrimData(sw,0,3)
...
##### Subplot 4: GRF #####
sp4 = PlotTransitionLinesColor(t_all, GRF[:,2], swplot, "",L"t, \text{[s]}", L"GRF, \text{[N]}")
...
sp10 = PlotSetUp(t_all, -p.kₛ*u_all[:,10] - p.cₛ*u_all[:,12], GRFd, swstance, " ",L"t, \text{[s]}",L"GRF_d, \text{[N]}")
sp10 = plot!(t, GRFd)
sp10 = PlotTransitionColor!(t_all, -p.kₛ*u_all[:,10] - p.cₛ*u_all[:,12], swstance)
...
##### Subplot 14: GRF #####
sp14 = PlotTransitionLinesColor(t_all, -p.kₛ*u_all[:,10] - p.cₛ*u_all[:,12], swplot, "",L"t, \text{[s]}", L"GRF, \text{[N]}")
